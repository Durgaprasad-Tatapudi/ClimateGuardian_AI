...
\end{figure}
The operational LightGBM model evaluating the Heatwave hazard demonstrates strong discrimination on the held-out 2016--2018 period. The model achieves an Accuracy of 97.90\%, Precision of 85.58\%, Recall of 91.75\%, an $F_1$-score of 0.8856, an ROC-AUC of 0.9966, and a PR-AUC of 0.9677, with a Brier Score of 0.0159. The associated ROC curve (Fig.~\ref{fig:heatwave_roc}) visually confirms this robust discrimination under the held-out test distribution. Critically, these results are derived exclusively from the leakage-remediated 41-feature configuration. The initial model iteration relied on deterministic proxy variables directly associated with the target definition; following their explicit removal, the model retained its performance, ensuring that predictions map underlying drivers rather than tautological indicators. 

\subsection{Compound-Risk Results and Operational Selection}
Predicting Compound hazards requires differentiating offline theoretical capacity from operational deployment feasibility (Table~\ref{tab:op_selection}). The Logistic Regression Stacking Ensemble achieved the strongest predictive discrimination for the Compound target, recording an $F_1$-score of 0.4353, an ROC-AUC of 0.9553, and a PR-AUC of 0.4077. In comparison, the GRU offline benchmark achieved an $F_1$-score of 0.3293, an ROC-AUC of 0.9423, and a PR-AUC of 0.2965. 
